...
create_url() {
  ...
  echo "https://github.com/bazelbuild/bazel/releases/download/${version}/bazel-${version}${flavour}-installer-${INSTALLER_PLATFORM}.sh"
}
...
install_bazel() {
  ...
  "${installer}" \
    --base="${destination}" \
    --bin="${destination}/bin" \
    --bazelrc="${destination}/bin/bazel-real.bazelrc"
}

# Get the version of latest BAZEL
if [ -z "${BAZEL_VERSION}" ]; then
  BAZEL_VERSION=$(curl -I https://github.com/bazelbuild/bazel/releases/latest | grep '^Location: ' | sed 's|.*/||')
fi
...
install_bazel "$(find $PWD/bazel-installer -name '*.sh' | \
  ...
  ~/.bazel/HEAD
install_bazel "$(find $PWD/bazel-installer -name '*.sh' | \
  fgrep "PLATFORM_NAME=${INSTALLER_PLATFORM}" | fgrep jdk7 | head -1)" \
  ~/.bazel/HEAD-jdk7
...
ln -s ~/.bazel/latest ~/.bazel/${BAZEL_VERSION}
ln -s ~/.bazel/latest-jdk7 ~/.bazel/${BAZEL_VERSION}-jdk7
