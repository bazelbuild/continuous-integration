...
def print_modules_need_to_migrate(failed_jobs_per_flag):
    info_text = ["#### The following modules need migration"]
    jobs_need_migration = {}
    ...
    s1 = "" if module_num == 1 else "s"
    s2 = "s" if module_num == 1 else ""
    info_text.append(
        f"<details><summary>{module_num} module{s1} need{s2} migration, click to see details</summary><ul>"
    )
    ...
    bazelci.execute_command(
        [
            ...
            "annotate",
            "--append",
            "--context=modules_need_migration",
            "--style=error",
            f"\n{info_str}\n",
        ]
    )
...
def merge_jobs(jobs):
    """Merge jobs by module versions"""
    jobs_per_module = collections.defaultdict(list)
    for job in sorted(jobs, key=lambda s: s["name"].lower()):
        module, platform = get_module_and_platform(job)
        jobs_per_module[module].append(get_html_link_text(platform, job["web_url"]))
    return jobs_per_module


def merge_and_format_jobs(jobs, line_pattern):
    # Merges all jobs for a single module version into one line.
    # Example:
    #   module (platform1)
    #   module (platform2)
    #   module (platform3)
    # with line_pattern ">> {}: {}" becomes
    #   >> module: platform1, platform2, platform3
    jobs_per_module = merge_jobs(jobs)
    return [
        line_pattern.format(module, ", ".join(platforms))
        for module, platforms in jobs_per_module.items()
    ]


def get_module_and_platform(job):
    name = job["name"]
    platform = ""
    ...
    module = extract_module_version(name)
...
def print_result_info(already_failing_jobs, failed_jobs_per_flag):
    ...
    print_modules_need_to_migrate(failed_jobs_per_flag)
...
def main(argv=None):
    ...
    parser = argparse.ArgumentParser(
        description="Script to aggregate `bazelisk --migrate` test result for incompatible flags and generate pretty Buildkite info messages for BCR Bazel Compatibility Test pipeline."
    )
