# Dynamic Hermetic RBE Toolchain Generator (`bazel_ci_rules`)

This folder contains the unified, GCS-free, and hermetic Remote Build Execution (RBE) toolchain generator rule (`rbe_config`). 

It automates the generation of RBE toolchain configurations dynamically and hermetically on-the-fly, eliminating the maintenance overhead of pre-generating and uploading configurations to Google Cloud Storage (GCS) buckets when new Bazel versions are released.
...
- `--javabase` / `--host_javabase=@rbe_ubuntu//java:jdk`: Resolves Java compilations and host execution using the hermetic JDK detected inside the container.
