...
def prepare_test_module_repo(module_name, module_version, overwrite_bazel_version=None, root=None, suppress_log=False):
    ...
    # Use the highest Bazel version from the presubmit config rather than
    # "latest", because module graph resolution is version-sensitive (e.g.
    # bazel_tools bundles different dependency versions across Bazel majors).
    if overwrite_bazel_version:
        vendor_bazel_version = overwrite_bazel_version
    else:
        presubmit = yaml.safe_load(open(get_presubmit_yml(module_name, module_version), "r"))
        bazel_versions = set()
        if "matrix" in presubmit and "bazel" in presubmit.get("matrix", {}):
            bazel_versions.update(presubmit["matrix"]["bazel"])
        if "bcr_test_module" in presubmit:
            tm = presubmit["bcr_test_module"]
            if "matrix" in tm and "bazel" in tm.get("matrix", {}):
                bazel_versions.update(tm["matrix"]["bazel"])
        for task_config in presubmit.get("tasks", {}).values():
            if isinstance(task_config, dict) and "bazel" in task_config:
                v = task_config["bazel"]
                if isinstance(v, str) and not v.startswith("$"):
                    bazel_versions.add(v)
        if bazel_versions:
            # Pick the highest major version listed and request its latest minor
            # via "<major>.x". Versions in the matrix may use any of "9.x",
            # "9.0.0", or "9.0.0rc1"; comparing them as strings is not
            # semver-aware (e.g. "10.x" < "9.x" lexicographically). Comparing
            # by leading-integer major fixes that, and the trailing ".x"
            # bazelisk syntax avoids having to pick a specific minor.
            def major(v):
                m = v.split(".", 1)[0]
                return int(m) if m.isdigit() else 0
            vendor_bazel_version = "%d.x" % max(major(v) for v in bazel_versions)
        else:
